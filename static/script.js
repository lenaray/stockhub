    ...
    tickers.forEach(function (ticker) {
        $.ajax({
            ...
            success: function (data) {
                ...
                var flashClass;
                if (lastPrices[ticker] > data.currentPrices) {
...
function createChart(data) {
    var ctz = document.getElementById('tickerChart').getContext('2d');
    if (tickerChart) {
        tickerChart.destroy();
    }

    tickerChart = new CharacterData(ctz, {
        type: 'bar',
        data: {
            labels: data.tickers,
            datasets: [{
